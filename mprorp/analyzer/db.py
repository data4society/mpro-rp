...
def put_training_set_params(set_id, idf,  doc_index, lemma_index, object_features, session=None, commit_session=True):
    ...
    for doc_id in doc_index:
        ...
        features, indexes = object_features[doc_id]['values'], object_features[doc_id]['indexes']
...
def get_docs_by_markup_type(markup_type='51', session=None):
    ...
    res = session.query(Markup.document).filter((Markup.type == markup_type)).distinct().all()

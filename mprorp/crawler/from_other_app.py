...
from mprorp.controller.logic import VALIDATION_AND_CONVERTING_COMPLETE
from sqlalchemy.orm import load_only


def other_app_cloning(other_app_id, blacklist, url_domain, fields_to_clone, app_id, session):
    """download google news start feed and feeds for every story"""
    # download google news start feed
    origin_docs = session.query(Document).filter_by(app_id=other_app_id, status=VALIDATION_AND_CONVERTING_COMPLETE).all()
    docs = []

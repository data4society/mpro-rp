...
def add_rubric_to_doc(new_doc_ids, rubric_id, session=None):
    ...
    old_ids_2 = []
    for id in old_ids:
        old_ids_2.append(id[0])
    for new_doc_id in new_doc_ids:
        if new_doc_id not in old_ids_2:
...
def write_training_set(rubric_id, session=None):
 ...
 if session is None:
  session = Driver.db_session()
 new_set = TrainingSet(doc_ids=training_set, doc_num=len(training_set), name='Matvey_set_tr')
 session.add(new_set)
 session.commit()

def write_test_set(rubric_id, session=None):
 training_set = create_training_set(rubric_id)[1]
 if session is None:
  session = Driver.db_session()
 new_set = TrainingSet(doc_ids=training_set, doc_num=len(training_set), name='Matvey_set_test')
 session.add(new_set)
 session.commit()
